...
class SilentCameraHandler: NSObject {
	...
	func takeSilentPhoto(settings: [String: Any], result: @escaping FlutterResult) {
		ensureCameraPermission { [weak self] granted in
			...
			if #available(iOS 13.0, *) { photoSettings.photoQualityPrioritization = .quality }
	...
	func startSilentVideo(settings: [String: Any], result: @escaping FlutterResult) {
		...
		ensureCameraPermission { [weak self] granted in
			...
			func proceedWithSetup() {
				do { try self.setupCaptureSession(recordAudio: recordAudio) } catch {
					result(FlutterError(code: "SESSION_ERROR", message: error.localizedDescription, details: nil)); return
				}
				guard let movieOutput = self.movieOutput else {
					result(FlutterError(code: "NO_CAMERA", message: "Movie output not available", details: nil)); return
				}
				guard !movieOutput.isRecording else {
					result(FlutterError(code: "ALREADY_RECORDING", message: "Already recording", details: nil)); return
				}
				if self.captureSession?.isRunning == false { self.captureSession?.startRunning() }
			...
			if recordAudio {
				self.ensureMicrophonePermission { _ in proceedWithSetup() }
			} else { proceedWithSetup() }
		}
	}
	...
	func stopSilentVideo(result: @escaping FlutterResult) {
		...
		guard movieOutput.isRecording else {
			result(FlutterError(code: "NOT_RECORDING", message: "Not currently recording", details: nil)); return
		}
...
extension SilentCameraHandler: AVCapturePhotoCaptureDelegate {
	func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
		if let error = error {
			photoCompletionHandler?(FlutterError(code: "CAPTURE_ERROR", message: "Error capturing photo: \(error.localizedDescription)", details: nil))
			photoCompletionHandler = nil
			return
		}
		guard let imageData = photo.fileDataRepresentation() else {
			photoCompletionHandler?(FlutterError(code: "NO_DATA", message: "Could not get image data", details: nil))
			photoCompletionHandler = nil
			return
		}
		...
		do {
			...
			photoCompletionHandler?(FlutterError(code: "WRITE_ERROR", message: "Failed to write image: \(error.localizedDescription)", details: nil))
		}
		photoCompletionHandler = nil
	}
}
...
// Deprecated duplicate; intentionally left blank to avoid duplicate class definitions.
// See `SilentCameraChannel.swift` for the active implementation.
